...
public class frmPago extends javax.swing.JInternalFrame {
    private static final Logger logger = LoggerFactory.getLogger(frmConsumo.class);
...
void mostrar(String buscar) {
    try {
        ...
        logger.info("Datos mostrados correctamente para buscar='{}'", buscar);

    } catch (Exception e) {
        logger.error("Error mostrando datos para buscar='{}'", buscar, e);
        JOptionPane.showMessageDialog(this, "Error al mostrar los datos. Revise el log para más detalles.", "Error", JOptionPane.ERROR_MESSAGE);
    }
}
    ...
    private void initComponents() {
        ...
        btnImprimirComprobanteConsumo.setText("Generar Boleta");
...
if (!txtidpago.getText().equals("")) {
        ...
        try {
            System.setProperty("net.sf.jasperreports.compiler.class", "net.sf.jasperreports.compilers.JavaCompiler");

            // 🔧 Carga del archivo .jrxml desde recursos internos del proyecto
            InputStream reportStream = getClass().getResourceAsStream("/Reportes/rptComprobanteConsumo.jrxml");
            if (reportStream == null) {
                throw new FileNotFoundException("No se encontró el archivo rptComprobanteConsumo.jrxml en /Reportes/");
            }
            ...
            logger.info("Reporte comprobante consumo generado para idpago={}", txtidpago.getText());

        } catch (Exception e) {
            logger.error("Error generando reporte comprobante consumo para idpago={}", txtidpago.getText(), e);
            JOptionPane.showMessageDialog(this, "Error al generar el comprobante: " + e.getMessage(),
                                          "Error", JOptionPane.ERROR_MESSAGE);
        }
    } else {
        logger.warn("Intento de generar comprobante consumo sin idpago seleccionado");
        JOptionPane.showMessageDialog(this, "Seleccione un pago para imprimir el comprobante.",
